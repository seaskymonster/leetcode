public class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        if(length%2 == 1){
           return findfortheKth(nums1, nums2, length/2+1, 0, nums1.length-1, 0, nums2.length-1); // 如果总共有3个，找第2大
        }else{
          return (findfortheKth(nums1, nums2, length/2, 0, nums1.length-1, 0, nums2.length-1)
                 +findfortheKth(nums1, nums2, length/2+1, 0, nums2.length-1, 0, nums2.length-1))/2; //如果总共有4个，找第2大，和第3大。

        }
    }
    
    
    public int findfortheKth(int[] nums1, int[] nums2, int k, int s1, int e1, int s2, int e2){ // 找第k大。
        if(s1 > e1){
            return nums2[s2+k-1];
        }
        if(s2 > e2) {
